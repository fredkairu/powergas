...
class Notifications extends MY_Controller
{
    ...
    function getNotifications()
    {
        ...
        $this->datatables
            ->select("id,type,date, from_date, till_date,destination_emails,comment")
            ->from("notifications")
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                 ...
                 'destination_emails' => $this->input->post('destination_emails'),
     ...
     function addSms()
    {
        ...
        $this->form_validation->set_rules('smsnote', lang("smsnote"), 'required|min_length[3]');

        if ($this->form_validation->run() == true) {
            $data = array(
                 
                 'destination_number' => $this->input->post('destination_number'),
                'note' => $this->input->post('smsnote'),
       ...
       if ($this->form_validation->run() == true && $this->cmt_model->addSmsNotification($data)) {
            $this->session->set_flashdata('message', lang("sms_sent"));
            redirect("notifications");
        } else {
          $this->session->set_flashdata('message', lang("sms failed. check the phone numbers or internet connection"));
            redirect("notifications");
            }
            ...
            $this->data['error'] = validation_errors();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'notifications/sendsms', $this->data);
