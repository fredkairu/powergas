...
class Welcome extends MY_Controller
{
    ...
    {
       //   $this->site->checkModulePermission('dashboard');
        ...
        $volume=$this->purchases_model->getLPGGas();   
...
if(is_array(json_decode($stockcoverpso))){
    ...
    function si(){
          $this->site->checkModulePermission('si');
       $post=$this->input->post();
       $this->load->model('sales_model');
       $value="";
...
$forecast1fullyear=$this->sales_model->getBudgetForecastForMonth($data,$datemonthsbudget,"forecast","PSO");
    ...
    function sso(){
           $this->site->checkModulePermission('sso');
       $post=$this->input->post();
       
     //$this->output->cache(30);
      // DIE(print_r($post));
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['f_country'],"product"=>$post["products"],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"datefrom"=>$post['datefrom'],"dateto"=>$post['dateto'],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"period"=>$post["period"]);
 
    //die(dirname(__FILE__));
        
    $filters="";
    if(empty($data["period"])){
        $data=Array ("cluster" => Array ("0"=>"EAH"),"period"=>date("m")."-".date("Y"), "countrys" => Array ( "0" => 3,"1" => 6, "2" => 73, "3" => 76 ), "gbu" => "all", "promotion" =>"", "productcategoryfamily" =>"", "customer" =>"","grossnet" => 1 ,"ssocountry"=>"ssocountry", "period" =>Array(date("m")."-".date("Y") ) );
$_POST["ssocountry"]=$post["ssocountry"]="ssocountry";
$_POST["gsales"]=$post["gsales"]=1;
    }
   
    if($data){
         if(empty($data["period"])|| empty($data["countrys"])){
         $this->session->set_flashdata('error', $this->lang->line("please_select_period_and_country_first"));
            redirect($_SERVER["HTTP_REFERER"]);
    }
    $endmonth=end($data["period"]);
     
     if(substr($endmonth,-4)){
        $lastyeardate=date(substr($endmonth,-4)-1); 
     } else{
         $lastyeardate=date("Y")-1;
     }
    //die(print_r($lastyeardate));
     $countries=array();
   foreach ($data as $key=>$value){
       if(is_array($value)){
           if(strtolower($key)=="countrys"){
  ...
  //get sso sales per country for whole year and last year
   ...
   foreach ($countries as $valueid) {
       ...
       //die(print_r($data));
    $lastyearsalestotal= $this->sales_model->getYearSalesTotalConsolidated($data,$lastyeardate,$salestype);

    $thisyearsalestotal=$this->sales_model->getYearSalesTotalConsolidated($data,substr(end($data["period"]),-4),$salestype);
   
    $monthsalestotals=$this->sales_model->getSSOsalesByCountryConsolidated($data,"'".end($data["period"])."'");
    $lastyearmonth=  substr($data["period"][0],0,2)."-".(substr(end($data["period"]),-4) -1);
    
    $monthsalestotalslastyear=$this->sales_model->getSSOsalesByCountryConsolidated($data,"'".$lastyearmonth."'");
    //budget
     $countryy["thisyearbudgetmonth"]=$this->budget_model->getBudgetForMonthConsolidated($salestype,$data,end($data["period"]));
    ...
    $countryy["lastyearsales"]=round($lastyearsalestotal,2);
    $countryy["thisyearsales"]=round($thisyearsalestotal,2);
     $countryy["lastyearsalesmonth"]=round($monthsalestotalslastyear->resale/1000,2);
     $countryy["thisyearsalesmonth"]=round($monthsalestotals->resale/1000,2);
   ...
   //ytd
    ...
    $budgetlast=$this->sales_model->getYearBudgetTotalConsolidated($data,$lastyeardate,"SSO","budget");
       $countryy["lastyearbudget"]=$budgetlast;
   $budgetcurrent= $this->sales_model->getYearBudgetTotalConsolidated($data,substr(end($data["period"]),-4),"SSO","budget");
    $countryy["thisyearbudget"]=$budgetcurrent;
          $countryy["budgetytd"]=  $this->sales_model->ytdBudget($data,substr(end($data["period"])),"budget","SSO");
    //ytg
 ...
 //  die(print_r($countrydata));
        ...
        if(empty($data["productcategoryfamily"])|| in_array("all",$data["productcategoryfamily"])){
            ...
            $categories=$this->settings_model->getMinCategories($data,$post["batch"],$offset=($post["batch"]-20));   
   ...
   $budgetcurrent= $this->sales_model->getYearBudgetTotalConsolidated($data,substr(end($data["period"]),-4),"SSO","budget");
    $countryy["thisyearbudget"]=$budgetcurrent;
          $countryy["budgetytd"]= $this->sales_model->ytdBudget($data,substr(end($data["period"])),"budget","SSO"); //$this->sales_model->getBudgetForecastForMonth($data,$datemonthsbudget,"budget","SSO");
    //ytg
          $countryy["ytgsaleslastyear"]=$this->sales_model->ytgSalesSalesType($dataa,"SSO");
          $countryy["ytgsalesthisyear"]=$budgetcurrent-$countryy["ytdsalesthisyear"];
          //get remaining months and calculate ytg
...
$forecast1fullyear=$this->sales_model->getBudgetForecastForMonth($data,$datemonthsbudget,"forecast","SSO");
    ...
    function adhoc(){
       $post=$this->input->post();
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['f_country'],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"datefrom"=>$post['datefrom'],"dateto"=>$post['dateto'],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"product"=>$post["products"]);
    $filters="";
   foreach ($data as $key=>$value){
       if(is_array($value)){
           if(strtolower($key)=="countrys"){
    ...
    function monthlytrend(){
        $this->site->checkModulePermission('monthly_trend');
       $post=$this->input->post();
     //  die(print_r($post));
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['f_country'],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"period"=>$post["period"],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"product"=>$post["products"],"batch"=>$post["batch"],"sales_type"=>$post["sales_type"]);
    $filters="";
     // die(print_r($post["gsales"]));
     if(empty($data["period"])){
        $data=Array ("cluster" => Array (), "countrys" => Array (), "gbu" => "all", "promotion" =>"", "productcategoryfamily" =>"", "customer" =>"","grossnet" => 1 ,"ssocountry"=>"ssocountry", "period" => date("Y") ,"sales_type"=>"SSO");
$_POST["ssocountry"]=$post["ssocountry"]="ssocountry";
$_POST["gsales"]=$post["gsales"]=1;
    }
   foreach ($data as $key=>$value){
 ...
 if(empty($data["productcategoryfamily"])|| in_array("all",$data["productcategoryfamily"])){
     ...
     if($post["batch"]=="all"){$offset="all";}
     $offset=($post["batch"]-20);
     
             $categories=$this->settings_model->getMinCategories($data,$post["batch"],$offset);      
...
//    foreach ($alldates as $datemonth) {
    if($data["sales_type"]=="SSO"){
       $sale=$this->sales_model->getSSOsalesByCountryConsolidatedOnly($data,$lastyear,$startyear);
     }
        else{
         $sale=$this->sales_model->getPSOsalesByCountryYearly($data,$lastyear,$startyear);    
        }
   
 
    
$countryy["salestotals"]=$sale;
//      
//      $monthsalestotals[$datemonth]=round($sale->resale/1000,2); 
//      
//    }
...
$countryy["salestotals"]=$sale;
    ...
    array_push($countrydata, $countryy);
       ...
       // print_r($alldates);
 // die(print_r($countrydata));
        $this->data["sales_type"]=$data["sales_type"];
  $this->data["countries"]=$countrydata;
  ...
  $this->data["alldates"]=$alldates;
   $this->data["yearonly"]=$startyear;
    $this->data['currencies']=  $this->settings_model->getAllCurrencies();
     ...
     function sit(){
         $this->site->checkModulePermission('distributor_sit');
       $post=$this->input->post();
      
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['f_country'],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"period"=>$post["period"],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"product"=>$post["products"]);
    $filters="";
     ...
     foreach ($alldates as $datemonth) {
         ...
         if ($cat=="PSO"){
            ...
            $monthsalestotals[$datemonth]=$psosale;   
             ...
             //ok
            $sale= $this->sales_model->getSSOsalesByCountry($data,"'".$datemonth."'");
            $ssosale=round($sale->resale/1000,5);
...
$countryy["salestotals"]=$monthsalestotals;
    ...
    array_push($countrydata, $countryy);
         ...
         //die($datemonth);
               $graph["lastaverage"]=  $this->purchases_model->getAverageSales($data,$datemonth,"SSO");
              $stockcover=$this->purchases_model->getMonthStockCover($data,$datemonth,"SSO");
   ...
   $this->data["yearonly"]=$startyear;
    $this->data['currencies']=  $this->settings_model->getAllCurrencies();
     ...
     function distributorsit(){
       ...
       $post=$this->input->post();
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['f_country'],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"period"=>$post["period"],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"product"=>$post["products"],"valueqty"=>$post["valueqty"]);
    $filters=$data;
        
    //die(print_r($data));
   // die(print_r($data));
     if(empty($data["period"])){
        $data=Array ("cluster" => Array ( "0" => "EAH" ), "countrys" => Array ( "0" => 3,"1" => 6, "2" => 73, "3" => 76 ), "gbu" => "all", "promotion" =>"", "productcategoryfamily" =>"", "customer" =>"","grossnet" => 1 ,"ssocountry"=>"ssocountry", "period" => date("m-Y"),"valueqty"=>"value" );
$_POST["ssocountry"]=$post["ssocountry"]="ssocountry";
$_POST["gsales"]=$post["gsales"]=1;
    }
 ...
 //die(print_r($newdates));
  ...
  $countries=  implode(",", $data['countrys']);
  ...
  $alldates=explode(",",$dates["allyears"]);
   ...
   foreach($data['countrys'] as $countryid){
    ...
    $this->db->select('country,country_id,distributor,distributor_id,brand_id');
    $this->db->where("upload_type","SALE");
    $this->db->where("country_id in('".$countryid."')");
  ...
  foreach ($q->result() as $row) {
   // $currency=$this->settings_model->getCurrencyByID($countryid);  
           $graph["country_name"]=$row->country;
           $graph["customer_name"]=$row->distributor;
          // $graph["brand"]=$row->brand;
           $graph["distributor_id"]=$row->distributor_id;
           $graph["country_id"]=$row->country_id;
           $graph["brand_id"]=$row->brand_id;
//           $stock=$this->sales_model->getClosingSaleDitNew($filters,$dateperiod,"SSO");
           //$graph["value"][$row->country][$row->distributor][$row->brand][$dateperiod]=$stock["value"]; 
     //$graph["qty"][$row->country][$row->distributor][$row->brand]=$stock["qty"];// $this->purchases_model->getClosingStockDitNew($filters,$dateperiod,"SSO");
   // echo( $graph["value"][$currency->country][$company->name][$productname][$cat->category_name][$dateperiod])."<br>";
    array_push($graphdata,$graph);
     }
   }
     //$stockss=$this->purchases_model->getClosingStockDitNew($filters,$currentmonthyear,"SSO");
     //  $graph["value_stock"][$currency->country][$company->name][$productname][$cat->category_name][$currentmonthyear]=$stockss["value"];
//$graph["value_stock"][$currency->country][$company->name][$productname][$cat->category_name][$currentdatemonth]=$this->purchases_model->getClosingStockDitNew($filters,$currentdatemonth,"SSO");
...
//       
//       $filters=array("gmid"=>$prdetail->code,"country_name"=>$currency->country,"distributor"=>$customerid);
...
//     $graph["qty"][$currency->country][$company->name][$productname][$cat->category_name][$dateperiod]=$stock["qty"];// $this->purchases_model->getClosingStockDitNew($filters,$dateperiod,"SSO");
//   // echo( $graph["value"][$currency->country][$company->name][$productname][$cat->category_name][$dateperiod])."<br>";
//     }
//     $stockss=$this->purchases_model->getClosingStockDitNew($filters,$currentmonthyear,"SSO");
//       $graph["value_stock"][$currency->country][$company->name][$productname][$cat->category_name][$currentmonthyear]=$stockss["value"];
...
//    }
   ...
   // die(print_r($filters));
       ...
       if($data["valueqty"]=="value"){
        ...
        $this->page_construct('distributorsit', $meta, $this->data);
           ...
           $bc = array(array('link' => '#', 'page' => lang('Distributor_SIT_Qty')));
        $meta = array('page_title' => lang('Distributor_SIT'), 'bc' => $bc);
 ...
 function distributorsitqty(){
   ...
   // die(print_r($data));
     if(empty($data["period"])){
        $data=Array ("cluster" => Array ( "0" => "EAH" ), "countrys" => Array (0=>3), "gbu" => "all", "promotion" =>"", "productcategoryfamily" =>"", "customer" =>"","grossnet" => 1 ,"ssocountry"=>"ssocountry", "period" => date("m-Y") );
$_POST["ssocountry"]=$post["ssocountry"]="ssocountry";
$_POST["gsales"]=$post["gsales"]=1;
    }
 ...
 //die(print_r($newdates));
  ...
  $alldates=explode(",",$dates["allyears"]);
    ...
    $this->db->where("upload_type","SALE");
    $this->db->where("country_id in('".$countries."')");
    $this->db->group_by("distributor_id");
  ...
  //  $this->data['currencies']=  $this->settings_model->getAllCurrencies();
    ...
    function distributorsitdetailed(){
          $this->site->checkModulePermission('distributor_sit');
       $post=$this->input->get();
       $value="";
    $data=array("cluster"=>$post['cluster'],"countrys"=>$post['countrys'],"gbu"=>$post['gbu'],"promotion"=>$post['promotion'],"productcategoryfamily"=>$post['category'],"customer"=>$post["customer"],"period"=>$post["period"],"grossnet"=>$post["gsales"],"distributor"=>$post["distributor"],"price_type"=>$post["price_type"],"product"=>$post["products"],"valueqty"=>$post["valueqty"]);
    $filters=$data;
        
    //die(print_r($data));
   // die(print_r($data));
     if(empty($data["period"])){
        $data=Array ("cluster" => Array ( "0" => "EAH" ), "countrys" => Array (0=>3), "gbu" => "all", "promotion" =>"", "productcategoryfamily" =>"", "customer" =>"","grossnet" => 1 ,"ssocountry"=>"ssocountry", "period" => date("m-Y"),"valueqty"=>"value" );
$_POST["ssocountry"]=$post["ssocountry"]="ssocountry";
$_POST["gsales"]=$post["gsales"]=1;
    }
 ...
 // die(print_r($thisyears));
  ...
  $alldates=explode(",",$dates["allyears"]);
    $alldates=  array_merge($alldates, $newdates);
  // die($data["countrys"]);
    $countries=  explode(",", $data["countrys"]);
    if($post["distributor"] && !$post["distributor"]="undefined"){
          $distributors=  implode(",", $data["distributor"]);
      $this->db->where("distributor_id IN ('".$distributors."')");;  
    }
foreach($alldates as $date){
     
    $this->db->select('country,country_id,distributor,distributor_id,brand,brand_id,SUM(gross_sale) as sale');
    $this->db->where("upload_type","SALE");
    $this->db->where("country_id in('".$data["countrys"]."')");
     $this->db->where("DATE_FORMAT(monthyear,'%m-%Y') ='".$date."'");
    $this->db->group_by("brand_id");
  ...
  //  $this->data['currencies']=  $this->settings_model->getAllCurrencies();
    $this->data['clusters']=  $this->cluster_model->getClusters();
       ...
       if($data["valueqty"]=="value"){
        $bc = array(array('link' => '#', 'page' => lang('Distributor_SIT')));
        $meta = array('page_title' => lang('Distributor_SIT'), 'bc' => $bc);
        $this->page_construct('distributorsitdetailed', $meta, $this->data);
       } else{
           $bc = array(array('link' => '#', 'page' => lang('Distributor_SIT_Qty')));
        $meta = array('page_title' => lang('Distributor_SIT'), 'bc' => $bc);
        $this->page_construct('distributorsitqtydetailed', $meta, $this->data);
       }
        
    }
    
  function msranalysis(){
        $this->site->checkModulePermission('msr_summary');
       $post=$this->input->get();       
...
foreach ($periods as $value) {
  ...
  $newdates=ltrim($dates,",");
   ...
   $post["countrys"]=  explode(",",$post["countrys"]);
   if(isset($post["cluster"])){
      foreach ($post["countrys"] as $value) {
				if($value){
                $clusters.="'".$value."',";
				}
                     }
...
$data=array("cluster"=>$post['cluster'],"countrys"=>$valuee,"gbu"=>$post['gbu'],"period"=>$newdates,"gsales"=>$post["gsales"],"gross_net"=>$post["gsales"]);
}
 
 //die(print_r($data));
 
  
    $teams=$this->site->getAllTeams($data);
  // die(print_r($teams));
   ...
   //  print_r($alignments);
      $align["team_name"]=$team->name;
        ...
        $align["sf_name"]=$value->msr_alignment_name;
        $align["sf_id"]=$value->id;
        $align["alignment_products_customers"]=$this->site->getAlignmentCustomersAndProducts($value->id);
      //$align["budget"]= 
       $graphdata[]=$align;
        }
  ...
  $this->data["newdates"]=$newdates;
   ...
   $month_name = date("F", mktime(0, 0, 0, substr($currentmonthyear,0,2), 10)); 
    ...
    $this->data['currencies']=  $this->settings_model->getAllCurrencies();
        ...
        $bc = array(array('link' => '#', 'page' => lang('MSR Detailed Analysis')));
        $meta = array('page_title' => lang('MSR Detailed Analysis'), 'bc' => $bc);
        $this->page_construct('msr', $meta, $this->data);
        
    }
    
     function msrsummaryanalysis(){
           $this->site->checkModulePermission('msr_summary');
       $post=$this->input->post();       
...
foreach ($periods as $value) {
  ...
  // die(print_r($post));
   if(isset($post["cluster"])){
      foreach ($post["f_country"] as $value) {
				if($value){
                $clusters.="'".$value."',";
				}
                     }
...
$data=array("cluster"=>$post['cluster'],"countrys"=>$valuee,"gbu"=>$post['gbu'],"period"=>$newdates,"net_gross"=>$post["gsales"]);
}
 ...
 //die(print_r($newdates));
    ...
    foreach ($teams as $team){
      $alignments=$this->site->getTeamAlignments($team->id);
      $align["team_name"]=$team->name;
      $align["team_id"]=$team->id;
      $align["team_members"]=$alignments;
    // print_r($alignments);
//      $alignn=array();
//        foreach ($alignments as $value) {
//            
  ...
  $this->data["newdates"]=$newdates;
   ...
   $month_name = date("F", mktime(0, 0, 0, substr($currentmonthyear,0,2), 10)); 
    ...
    $this->data['currencies']=  $this->settings_model->getAllCurrencies();
        ...
        $bc = array(array('link' => '#', 'page' => lang('MSR Summary Analysis')));
        $meta = array('page_title' => lang('MSR Summary Analysis'), 'bc' => $bc);
        $this->page_construct('msrsummary', $meta, $this->data);
