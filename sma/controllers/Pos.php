...
class Pos extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        $detail_link = anchor('pos/view_complete/$1', '<i class="fa fa-file-text-o"></i> Print Receipt'/* . lang('view_receipt')*/);
       ...
       // $add_mark_link=anchor('#', '<i class="fa fa-money"></i> Mark Payment' , 'class="markpayment" data-id="$1" onclick="return false"');
        ...
        if ($warehouse_id) {
           // die("&&&&");
             $this->datatables
                ->select($this->db->dbprefix('sales').".id as id, date, reference_no, biller,cashier,customer, grand_total, paid, (grand_total-paid) as balance, payment_status, companies.email as cemail")
                ->from('sales')
...
//                ->group_by('sales.id');
        ...
        if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin) {
          // $this->datatables->where('cashier_id', $this->session->userdata('user_id'));
...
function getDeletedSales($warehouse_id = NULL)
    {
      ...
      if ($warehouse_id) {
         $this->datatables
                ->select("deleted_sales.id as id, deleted_sales.date,deleted_sales.reference_no,deleted_sales.sales_type,products.code,products.name,deleted_sales.customer,products.business_unit,categories.name as catname,deleted_sales.sales_cluster,deleted_sales.sales_region")
                   ->join('sale_items', 'sale_items.sale_id=deleted_sales.id', 'left')
...
//        $this->form_validation->set_rules('chefs', 'Chefs', 'required');
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($sid) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $productquantities.=$item->quantity."*".$item->product_name."#";
    ...
    function close_register($user_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['ccsales'] = $this->pos_model->getRegisterCCSales($register_open_time,$closetime, $user_id);
            $this->data['cashsales'] = $this->pos_model->getRegisterCashSales($register_open_time,$closetime, $user_id);
            $this->data['mpesasales'] = $this->pos_model->getRegisterMPESASales($register_open_time,$closetime);
            $this->data['costcentersales'] = $this->pos_model->getRegisterCostCenterSales($register_open_time,$closetime);
            $this->data['chsales'] = $this->pos_model->getRegisterChSales($register_open_time,$closetime, $user_id);
            $this->data['pppsales'] = $this->pos_model->getRegisterPPPSales($register_open_time,$closetime, $user_id);
            $this->data['stripesales'] = $this->pos_model->getRegisterStripeSales($register_open_time,$closetime, $user_id);
            $this->data['totalsales'] = $this->pos_model->getRegisterSales($register_open_time,$closetime, $user_id);
            $this->data['refunds'] = $this->pos_model->getRegisterRefunds($register_open_time,$closetime);
            $this->data['cashrefunds'] = $this->pos_model->getRegisterCashRefunds($register_open_time,$closetime);
            $this->data['expenses'] = $this->pos_model->getRegisterExpenses($register_open_time,$closetime);
...
//            }
    ...
    function register_details()
    {
        $this->sma->checkPermissions('index');
        $register_open_time = $this->session->userdata('register_open_time');
        $closetime=date("Y-m-d 23:59");
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['ccsales'] = $this->pos_model->getRegisterCCSales($register_open_time,$closetime);
        $this->data['cashsales'] = $this->pos_model->getRegisterCashSales($register_open_time,$closetime);
        $this->data['chsales'] = $this->pos_model->getRegisterChSales($register_open_time,$closetime);
        $this->data['mpesasales'] = $this->pos_model->getRegisterMPESASales($register_open_time,$closetime);
        $this->data['costcentersales'] = $this->pos_model->getRegisterCostCenterSales($register_open_time,$closetime);
        $this->data['pppsales'] = $this->pos_model->getRegisterPPPSales($register_open_time,$closetime);
        $this->data['stripesales'] = $this->pos_model->getRegisterStripeSales($register_open_time,$closetime);
        $this->data['totalsales'] = $this->pos_model->getRegisterSales($register_open_time,$closetime);
        $this->data['refunds'] = $this->pos_model->getRegisterRefunds($register_open_time,$closetime);
        $this->data['expenses'] = $this->pos_model->getRegisterExpenses($register_open_time,$closetime);
...
$result=$this->db->query("select name from sma_companies where customer_group_name like '%corporate%' ")->result();
  ...
  $this->data["costcenter"]= $result;
    ...
    function install_update($file, $m_version, $version)
    {
     ...
     function closes_register($user_id,$date="",$closetime="")
    {
         if($_REQUEST["todate"]){
             $closetime=date("Y-m-d 23:59",  strtotime($_REQUEST["todate"]));
         }
         else{
         $closetime=date("Y-m-d 23:59");
         }
          if($_REQUEST["fromdate"]){
     $date= date("Y-m-d 00:01",  strtotime($_REQUEST["fromdate"]));        
          }
          else{
        $date=date("Y-m-d 00:01");
          }
      $this->sma->checkPermissions('index');
        if (!$this->Owner && !$this->Admin) {
            $user_id = $this->session->userdata('user_id');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('closed_at' =>$closetime,
                'total_cash' => $this->input->post('total_cash'),
                'total_cheques' => $this->input->post('total_cheques'),
                'total_cc_slips' => $this->input->post('total_cc_slips'),
                'total_cash_submitted' => $this->input->post('total_cash_submitted'),
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            if ($this->Owner || $this->Admin) {
                $user_register = $user_id ? $this->pos_model->registerData($user_id) : NULL;
                if(!$date){
                $register_open_time = date("Y-m-d H:i:s", strtotime("today 00:01"));//$user_register ? $user_register->date : $this->session->userdata('register_open_time');
            }
            else{$register_open_time=$date;}
                $this->data['cash_in_hand'] = $user_register ? $user_register->cash_in_hand : NULL;
                $this->data['register_open_time'] = $user_register ? $register_open_time : NULL;
            } else {
                 if(!$date){
                $register_open_time =date("Y-m-d H:i",strtotime("today 00:01"));// $this->session->userdata('register_open_time');
                 }
                 else{$register_open_time=$date;}
                $this->data['cash_in_hand'] = NULL;
                $this->data['register_open_time'] = NULL;
			...
			$q = $this->db->get_where('users', array('company_id' =>NULL,'active'=>1));
        if ($q->num_rows() > 0) {
