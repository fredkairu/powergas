...
class Distributor_product_model extends CI_Model
{
    ...
    if (!empty($products)) {
            foreach ($products as $product) {
               $q = $this->db->get_where('distributor_products', array('product_id' =>$product["product_id"],"distributor_id"=>$product["distributor_id"]), 1);
        if ($q->num_rows() > 0) {
           $this->db->update('distributor_products', $product,array('product_id' =>$product["product_id"],"distributor_id"=>$product["distributor_id"]));
        } else{
    ...
    public function getAllDistributorProducts()
    {
          ...
          $this->db->select('sma_customer_products_name_matching.id,sma_customer_products_name_matching.CUSTOMER_NAMING,sma_customer_products_name_matching.SANOFI_NAMING');
      $q = $this->db->get('sma_customer_products_name_matching');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
     ...
     public function getADistributorProduct($id)
    {
      ...
      $q = $this->db->get_where('sma_customer_products_name_matching',array('id'=>$id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	  ...
	  $trimmedname=  str_replace(" ","",$product_desc);	   
        $this->db->select('sma_distributor_products.product_id as id,sma_distributor_products.product_name as name,sma_products.type,sma_products.code')
			...
			->where(" REPLACE(sma_distributor_products.distributor_product_name,' ','')='".$trimmedname."'")
			  ...
			  ->where('sma_countryproductpricing.country_id',$country);
