...
//        return FALSE;
     ...
     public function getCategoryName($name)
    {
     ...
     function get_products_category ($category){
        $this->db->select('id,name');
    ...
    public function getProductDiscountsByProductID($id)
    {
        $this->db->select('sma_product_discounts.id as id, products.name as name,  sma_product_discounts.range_from ,
        sma_product_discounts.range_to, sma_product_discounts.discount, sma_product_discounts.loyalty')
            ->join('products', 'sma_product_discounts.product_id=products.id', 'left');
        $q = $this->db->get_where('sma_product_discounts', array('sma_product_discounts.product_id' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getProductDiscountsByProductIDAndDistributorID($product_id,$distributor_id)
    {
        $this->db->select('sma_product_discounts.id as id, products.name as name,  sma_product_discounts.range_from ,
        sma_product_discounts.range_to, sma_product_discounts.discount, sma_product_discounts.loyalty');
        $this->db->from('sma_product_discounts');
        $this->db->join('products', 'sma_product_discounts.product_id=products.id', 'left');
        $this->db->where('sma_product_discounts.product_id', $product_id);
        $this->db->where('sma_product_discounts.distributor_id', $distributor_id);
    ...
    public function getProductDiscountsByID($id)
    {
        $this->db->select('sma_product_discounts.id as id, products.name as name,  sma_product_discounts.range_from ,
        sma_product_discounts.range_to, sma_product_discounts.discount, sma_product_discounts.loyalty')
            ->join('products', 'sma_product_discounts.product_id=products.id', 'left');
        $q = $this->db->get_where('sma_product_discounts', array('sma_product_discounts.id' => $id),1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getProductDiscountByProductIDandQuantity($product_id,$quantity,$distributor_id)
    {
        ...
        $this->db->where('sma_product_discounts.product_id', $product_id);
        $this->db->where('sma_product_discounts.distributor_id', $distributor_id);
        $this->db->where(' "'.$quantity.'" BETWEEN sma_product_discounts.range_from AND sma_product_discounts.range_to');
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->row();
        }
       ...
       public function getProductByName($name)
    {
         ...
         $this->db->select("id,name,code,category_id,promoted");
        $this->db->where(" REPLACE(name,'\'','')='".$trimmedname."'");
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.code, ' . $this->db->dbprefix('products') . '.name, ' . $this->db->dbprefix('categories') . '.code as category_code, ' . $this->db->dbprefix('categories') . '.name as category_name, cost, price, quantity, alert_quantity')
    ...
    {
        $q = $this->db->get_where('products', array('code' =>trim($code)),1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    {
        if ($this->db->insert('sma_product_discounts', $data)) {
    ...
    protected function AddItemToErp($data){
...
//            );
        ...
        $saleinvoice["stock_id"]=$data["code"];
        $saleinvoice["category_id"]=1;
        $saleinvoice["tax_type_id"]=date("Y-m-d");
        $saleinvoice["description"]=$data["name"];
        $saleinvoice['units']=$data["unit"];
        $saleinvoice["mb_flag"]="B";
        $saleinvoice["sales_account"]="4010";
        $saleinvoice["cogs_account"]="5010";
        $saleinvoice["inventory_account"]="1510";
        $saleinvoice["adjustment_account"]="5040";
        $saleinvoice["wip_account"]="5040";
        $saleinvoice["dimension_id"]=1;
        $saleinvoice["material_cost"]="5040";
        $this->db->insert('stock_master',$saleinvoice);
        
        
      
         $this->db->insert('gl_trans',$saleinvoice);
         unset($saleinvoice);
    ...
    {
        //$id is id of product being edited,$prd->id is new product
        //die(print_r($data['code']));
        if($data['merge']){
            $oldprd=$this->getProductByID($id);
            $oldcode=$oldprd->code;
            $prd=$this->getProductByCode($data['code']);
          //die(print_r($prd));
           //update pricing
             $this->db->update('countryproductpricing',array("product_id"=>$prd->id,'product_name'=>$prd->name), array('id' => $id));
            
           
            $this->db->delete('products',array('id' =>$id)) ;//remove current product and replace with existing
            //mapping **8
           $this->db->update('distributor_products',array("product_id"=>$prd->id,'product_name'=>$prd->name), array('product_id' => $id));
            //cutomer-alignments***
           $this->db->update('customer_alignments',array("product_id"=>$prd->id,'products'=>$prd->name), array('product_id' => $id));
                 //update sales
           $this->db->update('sales',array("product_id"=>$prd->id,'products'=>$prd->name), array('product_id' => $id));
//sale items
                  $this->db->update('sale_items',array("product_id"=>$prd->id,'product_name'=>$prd->name,'product_code'=>$prd->code), array('product_id' => $id));
           

//update stck
              $this->db->update('purchase_items',array("product_id"=>$prd->id,'product_name'=>$prd->name,'product_code'=>$prd->code), array('product_id' => $id));
            //update budget
             $this->db->update('consolidated_sales_sso',array("gmid"=>$prd->code,'product_name'=>$prd->name), array('gmid' =>trim($oldcode)));
            //consolidated sso
