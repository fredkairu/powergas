...
$this->db->select('sma_users.id, sma_users.username, sma_users.email, sma_users.avatar, sma_users.created_on, sma_users.active, sma_users.first_name, sma_users.last_name, sma_users.company, sma_users.phone, sma_users.group_id, sma_users.company_id,sma_users.stock,');
        $y = $this->db->get_where('sma_users',array('sma_users.email'=>$email,'sma_users.active'=>'1'));
        if ($y->num_rows() > 0) {
           if($y->row()->group_id == 14){

               return $y->row();
           }else{
               $this->db->select('sma_users.id, sma_companies.id as salesman_id, sma_users.username, sma_users.email, sma_users.first_name,
         sma_users.last_name, sma_users.phone, sma_users.avatar, sma_companies.distributor_id,sma_users.stock,
          sma_companies.vehicle_id, sma_vehicles.plate_no, sma_vehicle_route.route_id, sma_vehicles.discount_enabled, sma_users.group_id, sma_groups.name')
                   ...
                   ->join("sma_groups","sma_users.group_id=sma_groups.id","left")
               ...
               $q = $this->db->get_where('sma_users',array('sma_users.email'=>$email,'sma_vehicle_route.day'=>$day));
               if ($q->num_rows() > 0) {
                   return $q->row();
               }
               return null;
    ...
    public function getUserWithCompanyID($company_id){
        $q = $this->db->get_where('users', array('company_id' => $company_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
