...
class Reports_model extends CI_Model
{
      ...
      public function getCustomerDueSales($customer_id)
    {
        ...
        $this->db->where('customer_id', $customer_id);//->where('payment_status',"pending");
        $q = $this->db->get('sales');
       if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getAgeingCustomer($id,$days=0){
        $date=date("Y-m-d H:i:s");
        $SQL="Select SUM(grand_total-paid) as balance from sma_sales where customer_id=$id  and (grand_total-paid)> 0 limit 1";
        $query = $this->db->query($SQL);
       return $query->result_array();
//      $this->db->where('customer_id', $id)->where('(grand_total-paid) > 0');
//        $q = $this->db->get('sales');
//        $total=0;
//       if ($q->num_rows() > 0) {
//            foreach (($q->result()) as $row) {
//                $total += $row->grand_total-$row->paid;
//            }
//            return $total;
//        }
    }   
