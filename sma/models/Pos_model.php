...
$array=array();
                ...
                if ($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {
                    ...
                    foreach ($item_costs as $item_cost) {
                        ...
                        if(! isset($item_cost['pi_overselling'])) {
							if(is_nan($item_cost['purchase_unit_cost'])){
							$item_cost['purchase_unit_cost']=0;
							}
							//die(print_r($item_cost));
     ...
     function postSaleInvoice($data,$customer=""){
        $this->db->set_dbprefix('0_');
//$this->db->dbprefix('tablename');
        //CR SALES
        $saleinvoice["type"]=10;
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=100101;
        $saleinvoice['memo_']="-POS sale";
        $saleinvoice["dimension_id"]=1;
        $saleinvoice["amount"]=-($data["grand_total"]-$data["total_tax"]);
        ...
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=1200;
        $saleinvoice['memo_']=$data["customer"]."-SALE";
        $saleinvoice["dimension_id"]=0;
        $saleinvoice["amount"]=($data["grand_total"]);
        $saleinvoice["person_type_id"]=$data["customer_id"];
         $this->db->insert('gl_trans',$saleinvoice);
     unset($saleinvoice);
         //CR VAT
         $saleinvoice["type"]=10;
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=2150;
        $saleinvoice['memo_']="VAT";
        $saleinvoice["dimension_id"]=0;
        $saleinvoice["amount"]=-$data["total_tax"];
         $this->db->insert('gl_trans',$saleinvoice);
     unset($saleinvoice);
       ...
       //payment DR Cash
        ...
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=1065;
        $saleinvoice['memo_']=$data["customer"];
        $saleinvoice["dimension_id"]=0;
        $saleinvoice["amount"]=($data["grand_total"]);
         $this->db->insert('gl_trans',$saleinvoice);
      unset($saleinvoice);
    ...
    public function getPortions($product_id)
    {
         ...
         $q = $this->db->query('SELECT id, portion1,portion1qty, portion2,
             ...
             portion5qty FROM sma_products WHERE id='.$product_id.' AND iskitchen=1');
                $j=5;
                $product_ids=array();
                $quantitys=array();
                $warehouse_ids=array();
...
$msgs."</b> in the Main Store Please Purchase");   
 ...
 return FALSE;
    ...
    public function reduceQuantity($product_id, $warehouse_id, $quantity)
    {
       ...
       // die($quantity." ".$warehouse_quantity->quantity);
    ...
    public function getTodaySales()
    {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ...
            ->where('type', 'received')->where('sma_payments.date  >', $date);
    ...
    public function getTodayCCSales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'CC');
    ...
    public function getTodayCashSales()
    {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'cash');
    ...
    public function getTodayRefunds()
    {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS returned', FALSE)
            ...
            ->where('type', 'returned')->where('sma_payments.date  >', $date);
    ...
    public function getTodayCashRefunds()
    {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS returned', FALSE)
            ->join('return_sales', 'return_sales.id=payments.return_id', 'left')
            ->where('type', 'returned')->where('sma_payments.date  >', $date)->where('paid_by', 'cash');
    ...
    public function getTodayChSales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'Cheque');
    ...
    public function getTodayMPESASales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'mpesa');
        ...
        public function getTodayCostCenterSales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'costcenter');
    ...
    public function getTodayPPPSales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'ppp');
    ...
    public function getTodayStripeSales()
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  >', $date)->where('paid_by', 'stripe');
    ...
    public function getRegisterSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"');
    ...
    public function getRegisterCCSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'CC');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getRegisterCashSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'cash');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getRegisterChSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'Cheque');
        $this->db->where('payments.created_by', $user_id);
 ...
 public function getRegisterMPESASales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'mpesa');
        $this->db->where('payments.created_by', $user_id);
     ...
     public function getRegisterCostCenterSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'costcenter');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getRegisterPPPSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'ppp');
        $this->db->where('payments.created_by', $user_id);
    ...
    public function getRegisterStripeSales($date,$enddate, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'stripe');
        $this->db->where('payments.created_by', $user_id);
            ...
            $status="paid";
                ...
                $status="due";
                 ...
                 $status="partial";
                    ...
                    $q=$this->db->query('UPDATE sma_sales SET total='
                            .$amount.', bill_change='
                            .$payment['bill_change'].', payment_status="'.$status.'",
