...
var confirmation=confirm("Do you want to edit Customer Distributor name?");
if(confirmation){
        $.ajax({
            
  url: "<?= site_url('customers/edit_cust_mapping') ?>",
...
var newname=$("#pname"+id).val();
var newcountry=$("#pcountry"+id+" :selected").val();
var confirmation=confirm("Do you want to delete Customer Distributor mapping?");
if(confirmation){
        $.ajax({
            
  url: "<?= site_url('customers/delete_cust_mapping') ?>",
  data:{"value": "<?= $this->security->get_csrf_hash() ?>","dp_id":id,"newname":newname,"newcountry":newcountry}
}).done(function(response) {
    alert(response);
 window.location.reload();
