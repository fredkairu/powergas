<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel"><?php echo lang('add_customer1'); ?></h4>
        </div>
        <?php $attrib = array('data-toggle' => 'validator', 'role' => 'form', 'id' => 'add-customer-form');
        echo form_open_multipart("customers/add1", $attrib); ?>
        <div class="modal-body">
            <p><?= lang('enter_info'); ?></p>
            <?php
                if($is_distributor==0){
                    echo "<label for='distributor_id'>Distributor</label>";
                    $distributorname=array();
                    foreach ($distributors as $distributor) {
                        $distributorname[$distributor->id] = $distributor->name;
                    }
                    echo form_dropdown('distributor_id', $distributorname, (isset($_POST['distributor']) ? $_POST['distributor'] : ($distributor ? $distributor->id : '')), 'class="form-control select" id="distributor_id" placeholder="' . lang("select") . " " . lang("distributor") . '" required="required" style="width:100%"');
        ...
        <div class="modal-footer">
            <?php echo form_submit('add_customer1', lang('add_customer1'), 'class="btn btn-primary"'); ?>
        </div>
    </div>
    <?php echo form_close(); ?>
</div>
