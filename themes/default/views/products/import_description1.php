        ...
        <div class="modal-body">
            ...
            <div class="row">
                <table id="PRData" class="table table-bordered table-condensed table-hover table-striped" width="90%">
                        <thead>
                            <tr class="primary"><th>#</th><th>Given Country</th><th>Customer Name</th><th>Customer Product Name</th><th>Action</th></tr></thead>
                <?php 
                $count=1;
...
$(document).ready(function(e){
   $("#distributorpricingg").click(function(e){
       ...
       // alert($("#country").val());
     $.ajax({
  url: "<?= site_url('products/customer_template') ?>",
  data:{"value": "<?= $this->security->get_csrf_hash() ?>","product_id":$("#productid").val()}
}).done(function(url) {
 window.open(url);
});
...
if(confirmation){
  ...
  url: "<?= site_url('products/add_mapping1') ?>",
