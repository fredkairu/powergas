<script>
    $(document).ready(function () {
        ...
        'copy', 'excel', 'pdf'
            ...
            'sAjaxSource': '<?= site_url('sales/getApprovedDiscounts'.($warehouse_id ? '/'.$warehouse_id : '')) ?>',
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, null,null,null,null,null,null,{"mRender": currencyFormat},null,null,{"mRender": row_status4}], //{"mRender": row_status} ,{"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": row_status}
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var Qty = 0, Amt = 0;
                for (var i = 0; i < aaData.length; i++) {
                    Qty += parseFloat(aaData[aiDisplay[i]][6]);
                    Amt += parseFloat(aaData[aiDisplay[i]][7]);
       
                }
                var nCells = nRow.getElementsByTagName('th');
                
                nCells[6].innerHTML = currencyFormat(parseFloat(Qty));
                nCells[7].innerHTML = currencyFormat(parseFloat(Amt));
                var nCells = nRow.getElementsByTagName('th');
             ...
             {column_number: 3, filter_default_label: "[<?=lang('distributor');?>]", filter_type: "text", data: []},
             {column_number: 4, filter_default_label: "[<?=lang('customer');?>]", filter_type: "text", data: []},
             {column_number: 5, filter_default_label: "[<?=lang('shop');?>]", filter_type: "text", data: []},
             {column_number: 8, filter_default_label: "[<?=lang('salesman');?>]", filter_type: "text", data: []},
             {column_number: 9, filter_default_label: "[<?=lang('vehicle');?>]", filter_type: "text", data: []},
             {column_number: 10, filter_default_label: "[<?=lang('Status');?>]", filter_type: "text", data: []},
            
              //{column_number:11, filter_default_label: "[<?=lang('Grand Total');?>]", filter_type: "text", data: []},
            ...
            //  {column_number:14, filter_default_label: "[<?=lang('Payment');?>]", filter_type: "text", data: []},
        ], "footer");
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-heart"></i><?= lang('Approved_Discounts') . ' ' . ($warehouse_id ? $warehouse->name : '') . ''; ?>
        </h2>

        <div class="box-icon">
            <ul class="btn-tasks">
                <a href="<?=site_url('reports/sales')?>" class="toggle_down tip" title="" data-original-title="Filter"><i class="icon fa fa-search"></i></a>
                <li class="dropdown">
                ...
                <div class="table-responsive">
                    <table id="SLData" class="table table-bordered table-hover table-striped">
                        ...
                        <tr>
                            ...
                            <th><?php echo $this->lang->line("Quantity Units"); ?></th>
