   ...
   $(document).ready(function () {
        ...
        fixedColumns:   {
            ...
            rightColumns:0
                ...
                .data()
                .reduce( function (a,b) {
                    //return Math.round((a + Number.EPSILON) * 100) / 100 + Math.round((b + Number.EPSILON) * 100) / 100;
...
<?php 
  ...
  <div class="searchform" style="z-index:999;top:10px;background-color:white;">
              <?php  echo  form_open_multipart("welcome/distributorsit", $attrib);?>
            <div class="row">
    ...
    <div class="form-group">
       ...
       // print_r($distributors);
                                                ...
                                                echo form_dropdown('distributor[]',$distributors,($_POST['distributor']), 'id="distributor" multiple="multiple" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("customer") . '"  style="width:100%;" ');
                                                ?>
            </div>  
        ...
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                ...
                <?php
                    ...
                    <div class="col-md-2">
                      <label class="ccs">Value/Qty</label>
                <?php
                                                $valueqty=array("value"=>"Value","qty"=>"Qty");
                                                echo form_dropdown('valueqty', $valueqty,($_POST['valueqty']), 'id="valueqty"  autocomplete="on" class="form-control input-tip select"   style="width:100%;" ');
                                                ?>
    ...
    <div class="col-md-4">
        <div class="form-group" style="padding-top:30px"><input type="submit" name="filter" value="Filter" class="btn btn-primary">&nbsp;<input type="submit" id="detailed" name="detailed" value="Det.Analysis" class="btn btn-primary">&nbsp;<input type="button" id="resetform" value="Reset" class="btn btn-info"></div>
                </div>
<?php echo form_close(); ?>
...
?>
      ...
      <div class="col-md-12" >
          <h4>Distributor SIT:Value<a href="#" class="exporttable2 pull-right">Export As Xls</a></h4>
            <div id="exceltable2">
         ...
         <table  class="SLData table table-bordered table-hover table-striped">
                        ...
                        <tr>
                           ...
                           <th class="headcol"><?php echo $this->lang->line("Distributor"); ?></th>
                           ...
                           <tr>
                              ...
                              <!--  <td><?=$ctry["brand"]?></td>-->
                               ...
                               <!--<td><?=$ctry["name"]?>  </td>-->
                                  ...
                                  <?php  foreach ($alldates as $month) {
                                      $salesvalues=$this->sales_model->getClosingSaleDitNew($filters,$month,$ctry["country_id"],$ctry["distributor_id"]);
                                     
                                      $salesvaluesarray[]=$salesvalues["value"];
                                      ?>
                                
                           
                               <td><?=round($salesvalues["value"],2)?></td>
                           <?php } ?>
                               <td><?php
                               $last3values = array_slice($salesvaluesarray,-3,3,true);  
                            $monthsales=round(array_sum($last3values)/3);
                              echo $monthsales;
                            $laststock=$this->purchases_model->getClosingStockDitNew($filters,$monthyear,$ctry["country_id"],$ctry["distributor_id"]);  
                               ?></td>    
                               <td><?=round($laststock["value"],2)?></td>
                               <td><?=round($laststock["value"]/$monthsales,2)?></td>
...
$('#cluster').change(function(){
   ...
   if(laststate !=""){
    ...
    $("#detailed").click(function(e){
        e.preventDefault();
        var gbu;
        if($("#f_country").val()==null){
            alert("Select at least one(1) country");
            return 0;
        }
        if($("#gbu").val()==null){
           gbu="";
   ...
   var geturl = "index.php/welcome/distributorsitdetailed?gbu=" +gbu+"&cluster="+$("#cluster").val()+"&countrys="+$("#f_country").val()+"&distributor="+$("#customer").val()+"&promotion="+$("#promo").val()+"&period="+$("#period").val()+"&gsales="+$("#gsales").val()+"&valueqty="+$("#valueqty").val() ;
