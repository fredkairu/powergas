...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php if ($inv) { ?>
        localStorage.setItem('sldate', '<?= date('m/Y',strtotime($inv->date))?>');
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("date", "sldate"); ?>
                                    <?php echo form_input('date', (isset($_POST['date']) ? $_POST['date'] : date('m/Y',strtotime($inv->date))), 'class="form-control input-tip datepicker monthPicker" id="sldate" required="required" placeholder="MM/YY"'); ?>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="col-md-4">
                                ...
                                <?php echo form_input('salestype', (isset($_POST['salestype']) ? $_POST['salestype'] : $inv->sales_type), 'class="form-control input-tip" id="salestype" required="required" readonly'); ?>
                                </div>
                            </div>
                        ...
                        foreach ($companies as $comp) {
                            $cp[$comp->id]  = $comp->name;
                        }
                             echo form_dropdown('customer', $cp,(isset($_POST['customer']) ? $_POST['customer'] : $inv->distributor_id), 'id="slcustomer" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("customer") . '" required="required" style="width:100%;" ');
                                                ?>
                                        </div>
                                    </div>
                                     <?php
                                     if($inv->sales_type=='PSO'){
                                         
                                     }else{
                                     ?>
                                   <div class="col-md-4"  id="custdiv">
                                            <div class="form-group">
                        ...
                        foreach ($sanoficustomer as $assoCust) {
                           // $country= $this->settings_model->getCurrencyByID($assoCust->country);
                            $ssoCust[$assoCust->id] = $assoCust->name;
                        }
                             echo form_dropdown('ssocustomer', $ssoCust,(isset($_POST['ssocustomer']) ? $_POST['ssocustomer'] : $inv->customer_id), 'id="ssocustomer" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("Customer") . '"  ');
                                                ?>
                                            </div>
                                    </div>
                                    <?php } ?>
									<div class="col-md-4" style='display:none'>
                                        ...
                                        <div class="form-group">
                                                <?= lang("Movement_Code", "mvntcode"); ?>
                                                <?php
                                                $MovemntCode[''] = "";
                                               
                        foreach ($movemntcodes as $amovemntcode) {
                            $MovemntCode[$amovemntcode->m_code] = $amovemntcode->m_code;
                        }
                             echo form_dropdown('mvntcode', $MovemntCode,(isset($_POST['mvntcode']) ? $_POST['mvntcode'] : $inv->movement), 'id="mvntcode" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("Movemnt_Code") . '" required="required" style="width:100%;" ');
                                                ?>
                                            </div>
                                    </div>
								<div class="col-md-4">
                        ...
                        foreach ($countries as $acountry) {
                            $country[$acountry->id ] = $acountry->country;
                        }
                                                                       echo form_dropdown('country', $country,(isset($_POST['country']) ? $_POST['country'] : $inv->country_id), 'id="country" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("country") . '" required="required" style="width:100%;" ');
                                                ?>
                                            </div>
                                    </div>
                                    <div class="col-md-4" style="display:none">
                                ...
                                <div class="controls table-controls">
                                    <table id="slTable"
                                        ...
                                        <tr>
                                            ...
                                            ?>
                                            <?php if($inv->sales_type =='SI')
                                            {
                                            ?>
                                            <th class="col-md-1"><?= lang("price"); ?></th>
                                            <?php } else {?>
                                            <th class="col-md-1"><?= lang("Resell_Price"); ?></th>
                                            <?php } ?>
