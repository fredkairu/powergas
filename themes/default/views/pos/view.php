...
<div id="wrapper">
 ...
 <h3 style="text-transform:uppercase;"><b>Eastbite Meat Palace LTD</b></h3>
			...
			$data1 = 'RECEIPT NO: '.$inv->id.''. PHP_EOL;
				...
				echo "PIN # - P051615194Q "." "."P.O Box 10538 - 00200";
           ...
           // echo "Chef" . ": " . $inv->chef . "</br>";
            ...
            <table class="table table-striped table-condensed" width="100%">
                ...
                foreach ($rows as $row) {
                    ...
                    echo '<tr><td colspan="2" style="font-size:14px">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') ;
                    $data7 = '#' . $r . ':  ' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '*' . $row->tax_code . '' . PHP_EOL;
					echo '&nbsp;(' . number_format($row->quantity) . ' x ';
                    ...
                    echo $this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)) .'<span class="pull-right">*' .$this->sma->formatMoney($row->subtotal) . '</span></tr>';
                ...
                <tfoot>
                <tr>
                    <th style="font-size:13px"><?= lang("total"); ?></th>
                    <th style="font-size:13px" class="text-right"><?= $this->sma->formatMoney($inv->total + $inv->product_tax); ?></th>
					<?php 
                ...
                ?>
                    ...
                    <tr>
                        <th style="font-size:13px"><?= lang("grand_total"); ?></th>
                ...
                if ($inv->paid < $inv->grand_total) { ?>
                 <!--   <tr>
                    ...
                    <tr>
                        <th style="font-size:13px"><?= lang("due_amount"); ?></th>
