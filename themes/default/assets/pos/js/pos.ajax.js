...
$('#posdiscount').focus(function () {
	 ...
	 * Edit Row Modal Hanlder
	 ...
	 $(document).on('click', '.edit', function () {
		...
		best_price = formatDecimal(row.children().children('.bestuprice').val()),
	 ...
	 * Edit Row Method
	 ...
	 $(document).on('click', '#editItem', function () {
		...
		var bestp = parseFloat($('#bstprice').val());
		if (site.settings.product_discount == 1 && $('#pdiscount').val()) {
			...
			if(price < bestp) {
				bootbox.alert("Selling Price Cannot be Less than the Best Price");
				return false;
			}
		}
...
function loadItems() {
  ...
  var best_price = item.row.best_price;
			...
			tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"> <input class="bestuprice" name="best_unit_price[]" type="hidden" value="' + item.row.best_price + '"> <input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val)) + '</span></td>';
      ...
      tr_html += '<td class="text-right"><span class="text-right sbestprice" id="bestprice_' + row_no + '">' + formatMoney(((parseFloat(best_price)) * parseFloat(item_qty))) + '</span></td>';
